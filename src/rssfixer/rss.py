...
def extract_links_html(soup, arguments):
    ...
    for entry in soup.find_all(arguments.html_entries):
        try:
            ...
            title = entry.find(arguments.html_title, re.compile(arguments.html_title_class)).text.strip()
        except (KeyError, AttributeError):
            print("ERROR: Unable to find URL or title in HTML element")
            sys.exit(1)
        try:
            description = entry.find(arguments.html_description, re.compile(arguments.html_description_class)).text.strip()
        except (KeyError, AttributeError):
...
def parse_arguments():
    ...
    parser.add_argument("--html-title-class", default="title", help="Flag to specify title class (regex)")
    parser.add_argument("--html-description", default="div", help="HTML selector for description")
    parser.add_argument("--html-description-class", default="summary" ,help="Flag to specify description class (regex)")
    ...
    parser.add_argument(
        "--json-title", default="title", help="JSON key for title"
    )
    parser.add_argument(
        "--json-description",
        default="preamble",
    ...
    parser.add_argument("--list", action="store_true", help="Find entries in HTML <ul>-list (default)")
